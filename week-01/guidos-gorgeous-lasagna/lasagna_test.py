...
# For this first exercise, it is really important to be clear about how
# we are importing names for tests. To that end, we are putting a try/catch
# around imports and throwing specific messages to help students decode that
# they need to create and title their constants and functions in a specific way.
try:
    from lasagna import (
        EXPECTED_BAKE_TIME,
        bake_time_remaining,
...
class LasagnaTest(unittest.TestCase):
    ...
    def test_preparation_time_in_minutes(self) -> None:
    ...
    def test_docstrings_were_written(self) -> None:
        ...
        for variant, function in enumerate(functions, start=1):
            with self.subTest(f'variation #{variant}', function=function):
                ...
                assert actual_result is not None, failure_msg
